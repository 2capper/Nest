...
- **Expanded comprehensive OBA team database to 50+ teams** - Built PostgreSQL database covering all major Ontario baseball affiliates including GTA (GTBA), London area (SCBA), Sun Parlour (SPBA), Kitchener-Waterloo (KWBA), Hamilton (HGBA), Niagara (NBBA), Ottawa (NCBA), and Thunder Bay (TBBA). Database now includes verified teams from Toronto Mets, Hamilton Cardinals, London Nationals, Kitchener Panthers, Ottawa Nepean, St. Catharines, and many others. Enables fast fuzzy matching for any Ontario team with sub-second search performance using local PostgreSQL queries (July 2025)
