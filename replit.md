...
- **Integrated COBA (Central Ontario Baseball Association) team database** - Added comprehensive COBA team scraper that imports all 159 teams from https://www.playoba.ca/stats#/2102/teams across all age divisions (10U-Senior) and skill levels (A, AA, AAA, B, C, D, DS, HS). Enhanced Team Discovery Scanner with both OBA range scanning and COBA affiliate scanning capabilities for complete Ontario baseball team coverage (July 2025)
...
- **Resolved critical authentication issues for production deployment** - Fixed authentication failures in deployed environments by optimizing cookie security settings for Replit deployments, implementing comprehensive debug logging for login attempts, and creating password reset utility for troubleshooting. Added dedicated Auth Debug tab in admin portal with password reset tool and system diagnostic capabilities. Simplified session configuration to use 'lax' sameSite policy and disabled secure-only cookies for better compatibility with various hosting environments (July 2025)
